...
# in order to remove eos and pad tag and add actulal line 
# part of data processing 
def filterData(sentence, eos = '<EOS>', pad = '<PAD>'):
    if eos in sentence:
...
# utility function to write result into file
def printOutput(predict_results, dic_dir):
    print 'Loading dictionary.........'
    with open(dic_dir) as dic:
        dic_file = pkl.load(dic)
    reversed_dic = dict((y,x) for x,y in dic_file.iteritems())
    
    print 'Creating new file and writing into it...'
    with open(FLAGS.pred_dir, 'w') as f:
        while True:
            try : 
                opt = predict_results.next()
                opt = opt['question'].tolist()
                if -1 in opt: # beam search
                    opt = opt[:opt.index(-1)]
                indices = [reversed_dic[index] for index in opt]
                sentence = ' '.join(indices)
                sentence = filterData(sentence)
...
def main(unused):
    ...
    # evaluate data
        ...
        printOutput(predict_results, FLAGS.dic_dir)

    
if __name__ == '__main__':

    # argument for different processing and setting up configuratons
